...
st.set_page_config(
  page_title="Dashboard AI & Kesejahteraan Mahasiswa - Single Page",
  page_icon="",
  layout="wide",
...
st.markdown("""
  ...
  /* Latar belakang halaman abu-abu sangat muda */
  .stApp {
    background-color: #F8F9FA;
  }
  
  /* Kartu KPI dengan Glassmorphism & Animasi Transisi Hover */
  .kpi-card {
    ...
    border-top: 5px solid #1F4E78; /* Navy header strip */
    text-align: center;
    margin-bottom: 20px;
...
if df is not None:
  st.markdown("<h1 style='text-align: center; color: #1F4E78;'> Dashboard Analisis Dampak AI Generatif Mahasiswa</h1>", unsafe_allow_html=True)
  st.markdown("<p style='text-align: center; color: #6C757D; font-size: 16px;'>Analisis performa akademik, perilaku belajar, dan kesejahteraan mental terhadap 50.000 mahasiswa.</p>", unsafe_allow_html=True)
  st.markdown("---")
  ...
  # Container filter dengan background putih
  ...
  # Baris tombol reset filter di bagian bawah box filter
  ...
  with col_fkpi1:
    st.markdown(f"<div style='text-align: center; border: 1px solid #E9ECEF; padding: 10px; border-radius: 8px; background-color: #FFFFFF;'><b>IPK Rata-rata Terfilter:</b> <span style='color: #1F4E78; font-weight: bold;'>{avg_gpa_f:.2f}</span></div>", unsafe_allow_html=True)
  with col_fkpi2:
    st.markdown(f"<div style='text-align: center; border: 1px solid #E9ECEF; padding: 10px; border-radius: 8px; background-color: #FFFFFF;'><b>Retensi Terfilter:</b> <span style='color: #4F81BD; font-weight: bold;'>{avg_ret_f:.1f}%</span></div>", unsafe_allow_html=True)
  with col_fkpi3:
    st.markdown(f"<div style='text-align: center; border: 1px solid #E9ECEF; padding: 10px; border-radius: 8px; background-color: #FFFFFF;'><b>Burnout Tinggi Terfilter:</b> <span style='color: #D62728; font-weight: bold;'>{burn_f:.1f}%</span></div>", unsafe_allow_html=True)
    
  st.markdown("<br>", unsafe_allow_html=True)
  
  # 2 Baris Visualisasi khusus
  col_vis1, col_vis2 = st.columns(2)
  
  with col_vis1:
    # Chart 1: GPA sebelum vs sesudah per segmen AI
    seg_order = ['Light User', 'Moderate User', 'Heavy User']
    
    # Tambahkan pengaman jika data kosong untuk filter tertentu
    if len(df_filtered) > 0:
      ...
      fig_gpa = px.bar(
        ...
        color_discrete_sequence=['#95A5A6', '#1F4E78']
      )
  ...
  # Baris berikutnya: Retensi Pengetahuan
  if len(df_filtered) > 0:
    fig_box = px.box(
      df_filtered,
      x='Perceived_AI_Dependency',
      y='Skill_Retention_Score',
      title="Sebaran Skor Retensi Pengetahuan per Skor Ketergantungan AI (Data Terfilter)",
  ...
  if len(df_filtered) > 0:
    ...
    # Ganti nama sumbu pembanding agar user-friendly
    label_map = {
    ...
    with col_comp1:
      ...
      # Komparasi Persentase Burnout Tinggi
  ...
  # ==========================================
  # ==========================================
  st.markdown("<h2 class='section-header'> Analisis Q6: Kesetaraan Akses Digital — Berbayar vs. Gratis</h2>", unsafe_allow_html=True)
  ...
  if len(df_filtered) > 0:
    ...
    # KPI perbandingan cepat
    col_q6_kpi1, col_q6_kpi2, col_q6_kpi3, col_q6_kpi4 = st.columns(4)
    with col_q6_kpi1:
      st.markdown(f"""
        <div class="kpi-card" style="border-top-color: #1F4E78;">
    ...
    with col_q6_kpi2:
      st.markdown(f"""
        <div class="kpi-card" style="border-top-color: #ADB5BD;">
    ...
    with col_q6a:
      # Bar chart IPK berbayar vs gratis per segmen AI
      ...
      for akses, color in [('Berbayar', '#1F4E78'), ('Gratis', '#ADB5BD')]:
    ...
    with col_q6b:
      # T-Test result + bar chart retensi
      if len(paid_f) > 1 and len(free_f) > 1:
        ...
        st.markdown(f"""
          <div style='background:#FFFFFF;padding:18px;border-radius:12px;border:1px solid #E9ECEF;'>
            <b style='color:#1F4E78;font-size:15px;'>Hasil Uji T-Test Independen</b><br><br>
            <b>IPK Akhir:</b> t = {t_gpa:.3f}, p = {p_gpa:.4f} &nbsp; {sig_gpa}<br>
            <b>Skor Retensi:</b> t = {t_ret:.3f}, p = {p_ret:.4f} &nbsp; {sig_ret}<br><br>
            <span style='font-size:12px;color:#6C757D;'>Signifikan jika p-value &lt; 0.05</span>
          </div>
        """, unsafe_allow_html=True)
      ...
      fig_q6_ret = px.bar(
        ...
        color='Label',
        color_discrete_map={'Berbayar': '#1F4E78', 'Gratis': '#ADB5BD'},
        text_auto='.1f'
      )
  ...
  # ==========================================
  # ==========================================
  st.markdown("<h2 class='section-header'> Analisis Q7: Faktor Dominan Keberhasilan Akademik</h2>", unsafe_allow_html=True)
  ...
  if len(df_filtered) > 10:
    ...
    # Bar chart korelasi terhadap IPK Akhir
    corr_target = corr['IPK Akhir'].drop('IPK Akhir').sort_values()
    colors_q7 = ['#D62728' if v < 0 else '#1F4E78' for v in corr_target.values]
    ...
    fig_corr_bar.update_layout(
      title="Q7: Peringkat Faktor Paling Dominan terhadap IPK Akhir",
    ...
    st.info(f"**Insight Q7:** Variabel **{top_positive}** memiliki korelasi positif terkuat terhadap IPK Akhir (r = {corr_target[top_positive]:.3f}), sementara **{top_negative}** memiliki korelasi negatif terkuat (r = {corr_target[top_negative]:.3f}).")

  else:
    st.warning("Data terlalu sedikit untuk analisis korelasi yang bermakna. Kurangi filter untuk melihat hasil Q7.")
